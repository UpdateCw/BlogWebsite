/**
 * gridScrollFx.js v1.0.0 http://www.codrops.com
 * 
...
(function(window) {
	...
	function inViewport(el, h) {
		var elH = el.offsetHeight, scrolled = scrollY(), viewed = scrolled
				+ getViewportH(), elTop = getOffset(el).top, elBottom = elTop
				+ elH,
		// if 0, the element is considered in the viewport as soon as it enters.
		// if 1, the element is considered in the viewport only when it's fully
		// inside
	...
	GridItem.prototype.addCurtain = function() {
		...
		var rgb = new ColorFinder(
				function favorHue(r, g, b) {
					return (Math.abs(r - g) * Math.abs(r - g) + Math.abs(r - b)
							* Math.abs(r - b) + Math.abs(g - b)
							* Math.abs(g - b)) / 65535 * 50 + 1;
				}).getMostProminentColor(this.image);
		if (rgb.r && rgb.g && rgb.b) {
			this.curtain.style.background = 'rgb(' + rgb.r + ',' + rgb.g + ','
					+ rgb.b + ')';
		}
	...
	GridItem.prototype.changeAnimationDelay = function(time) {
		...
		if (this.div) {
			this.div.style.WebkitAnimationDelay = time + 'ms';
			this.div.style.animationDelay = time + 'ms';
		}
	}
	...
	GridScrollFx.prototype.options = {
		...
		// visible in order for the animation to start
		...
		imagesLoaded(this.el, function() {
			...
			self.items.forEach(function(item) {
				if (inViewport(item.el)) {
					...
					item.addCurtain();
					// add random delay
					item.changeAnimationDelay(Math.random()
							* (self.options.maxDelay - self.options.minDelay)
			...
			if (!classie.has(item.el, 'shown')
				...
				var onEndAnimationFn = function(ev) {
					if (support.animations) {
						this.removeEventListener(animEndEventName,
								onEndAnimationFn);
					}
				...
				if (support.animations) {
					item.curtain.addEventListener(animEndEventName,
							onEndAnimationFn);
				} else {
					onEndAnimationFn();
				}
